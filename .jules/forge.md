...
## 2024-11-23 - DOMPurify Fallbacks for Safe HTML Construction
**Learning:** When using DOMPurify to construct safe HTML from templates containing variable data, wrap the raw HTML generation logic before sanitization. Always ensure the fallback condition (when DOMPurify is unavailable) degrades safely to a descriptive string rather than falling back to an unescaped DOM assignment.
**Action:** Isolate HTML construction to an intermediate variable (e.g. `rawHtml = ...`), sanitize it, and strictly assign the fallback to a static safe error string to ensure fail-closed security.
